    ...
    if ('geolocation' in navigator) {
      ...
      const watchId = navigator.geolocation.watchPosition(
        (position) => {
          ...
          // FOR MOBILE: Always show confirmation if manual request or far from campus
          const isMobile = /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent);
          
          if (isLikelyGPS && isInNigeria && isReasonablyNearCampus && !isManual && !isMobile) {
            setUserLocation({ lat: latitude, lng: longitude });
            console.log('✅ Auto-accepted: Good GPS near campus (desktop)');
          } else if (isManual && isLikelyGPS && isReasonablyNearCampus && !isMobile) {
            // Manual request near campus with good GPS - accept and notify (desktop only)
            setUserLocation({ lat: latitude, lng: longitude });
            showSuccess(
              'GPS Location Found',
            ...
            // - ANY request on mobile device
            console.log('🤔 Showing location confirmation - mobile:', isMobile, 'distance:', Math.round(distance/1000) + 'km, accuracy:', Math.round(accuracy) + 'm');
    ...
    <div className="min-h-screen bg-gradient-to-br from-indigo-900 via-purple-900 to-pink-800">
      ...
      <div className="relative h-[calc(100vh-3rem)] lg:h-[calc(100vh-4rem)]">
        ...
        <div className="hidden lg:block w-80 bg-white/10 backdrop-blur-lg border-r border-white/20 p-6 overflow-y-auto absolute left-0 top-0 bottom-0 z-10">
          ...
          {/* Desktop Route Info with Instructions */}
        ...
        {routeData && (
          <div className="lg:hidden absolute bottom-0 left-0 right-0 bg-white/95 backdrop-blur-lg border-t border-gray-200 rounded-t-2xl shadow-2xl z-20 transform transition-transform duration-300 max-h-96 overflow-hidden">
            <div className="p-4">
              {/* Handle Bar */}
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-center space-x-4 text-sm text-gray-600">
                  <div className="flex items-center space-x-1">
                    <span>📏</span>
                    <span>{routeData.distance < 1000 ? `${routeData.distance}m` : `${(routeData.distance / 1000).toFixed(1)}km`}</span>
                  </div>
                  <div className="flex items-center space-x-1">
                    <span>⏱️</span>
                  ...
                  <div className="mt-4">
                    ...
                    <div className="max-h-48 overflow-y-auto space-y-2">
                      {routeData.instructions.map((instruction, index) => (
                        <div
                          key={index}
                          className="flex items-start space-x-3 p-2 bg-blue-50 rounded-lg border-l-3 border-blue-400"
                        >
                ...
                <div className="mt-3 text-xs text-gray-500 bg-gray-50 p-2 rounded-lg">
                  {routeData.isCampusRoute ? (
                    <span>🎯 Smart Campus Routing • A* Algorithm</span>
                  ) : (
